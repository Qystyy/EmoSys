"""
EmoSys model inference test - updated 10/9/2026

This this the main inference code used to see the TFLite model performance. The code
...
# Run counter
# ------------------------------------------
run_file = "run_counter/emosys_run_counter.txt"
try:
...
# Class Labels -
# ------------------------------------------
# Pick based on which model is being used [FER / CE]
...
# Use for CE Model
print ("Loading labels...")
with open("labels_CE.json", "r") as f:
    data = json.load(f)
...
# FER model [TFLite form]
# Model is already quantise and run through knowledge distillation
print("Loading EmoSys FER model...")
interpreter = Interpreter(model_path="model/emosys_fer.tflite")


"""
# CE model [TFLite form]
...
GESTURE_MODEL_PATH = "model/gesture_model.tflite"
PERSONAL_MODEL_PATH = "model/gesture_model_personal.tflite"
...
# Gesture model run a different Interpretor than the emotion TFLite model
...
while True:
    ...
    if faces is not None:
        ...
        for idx, (fid, (x1, y1, x2, y2)) in enumerate(zip(face_ids, face_rects)):
            ...
            db.write_prediction(fid, emotion, confidence, posture_score, posture_label, gesture_label, gesture_confidence)
    ...
    # ================ Periodic Frame Capture & Data Push to Dashboard ================
    ...
    if (current_time_capture - last_capture_time >= CAPTURE_INTERVAL) and frame_preds:
        ...
        push_result(detected_emotion, confidence_score, frame_jpeg_bytes, inf_speed, gesture_label, gesture_confidence)
