...
# Run counter
run_file = "run_counter/CE_QT_run_counter.txt"
try:
...
# Logging
# -----------------------------
log_file = open("log/CE_emo_log.txt", "a", encoding="utf-8")
...
try:
    if platform.system() == "Linux":
        cpu_info = subprocess.check_output(
            "cat /proc/cpuinfo | grep 'Model name\\|Hardware\\|Model'",
...
# Rest is cross-platform via psutil
print(f"Cores     : {psutil.cpu_count(logical=False)} physical / {psutil.cpu_count(logical=True)} logical")
print(f"Freq      : {psutil.cpu_freq().max / 1000:.2f} GHz (max)")
...
with open("label_CE.json", "r") as f:
    data = json.load(f)
...
model_path="model/rafCE_mobilenetv2_qat.tflite"
...
# Specs for drawing
def draw_landmarks(frame, landmarks, connections, color=(0, 255, 0), connection_color=(255, 0, 0)):
...
def draw_habit_info(frame, habit_result, habit_level):
    # Behavior label
    behavior_color = BEHAVIOR_COLORS.get(habit_result.behavior, (255, 255, 255))

    # Starting position near the bottom-left
    x = 10
    y = h - 60

    cv2.putText(frame, f"{habit_result.behavior.value}",
                (x, y + 25), cv2.FONT_HERSHEY_SIMPLEX, 0.5, behavior_color, 2)

    cv2.putText(frame, f"Duration: {habit_result.duration_frames} frames",
                (x, y + 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
...
while True:
    ...
    if frame_count % 30 == 0:
        ...
        # habit detections
        if lm:
            habit_result = habit_detector.detect(lm, last_hand_landmarks)
            habit_level = habit_detector.get_habit_level()
            
            # Draw landmarks with behavior-specific colors
            if habit_result.is_active:
                color = BEHAVIOR_COLORS[habit_result.behavior]
            else:
                color = (0, 255, 0)  # Default green
            
            draw_landmarks(frame, lm, POSE_CONNECTIONS,
                        color=color, connection_color=(121, 22, 76))
    ...
    if faces is not None:
        for face in faces:
            ...
            # Experiment with the Calibration Logits
            #   - See the how the model perform with calibration first then edit it
            #   - Softmax is needed for the model count to be normalise
            #   - will be used when the accuracy reach 75% or more
            #   - current percentage is too low and more to guessing than predicting
            #   - this section will be close off till the time to use it
            """
            ...
            habit_label  = habit_result.behavior.value
            habit_cf     = round(habit_result.confidence, 2)
           ...
           # Face ID label
            ...
            # Draw habit info
            draw_habit_info(frame, habit_result, habit_level)
        ...
        tracker.update([])   # keep tracker ticking 
    ...
    if cv2.waitKey(1) & 0xFF in [ord('q'), ord('Q')]:
        ...
        if inference_times:
            ...
            with open("log/qat_tflite_inference_speed_log.txt", "a", encoding="utf-8") as speed_log:
